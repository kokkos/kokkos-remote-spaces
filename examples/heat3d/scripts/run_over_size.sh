...
VARS0="--bind-to socket -x NVSHMEMTEST_USE_MPI_LAUNCHER=1 -x NVSHMEM_SYMMETRIC_SIZE=10737418240"
...
#      mpirun -x CUDA_VISIBLE_DEVICES=0 -np 1 -npernode 1 $VARS0 $VARS1 $VARS2 -host $HOST1  ./$BENCHMARK -X $SIZE -Y $SIZE -Z $SIZE -N $ITERS | tee -a $FILENAME
#    done
#    let SIZE=$SIZE*2
# done
...
# for S in $(seq 1 7); do 
#    for reps in $(seq 1 3); do
#      mpirun -x CUDA_VISIBLE_DEVICES=0 -np 2 -npernode 1 $VARS0 $VARS1 $VARS2 -host $HOST1,$HOST2 ./$BENCHMARK -X $SIZE -Y $SIZE -Z $SIZE -N $ITERS | tee -a $FILENAME
#    done
#    let SIZE=$SIZE*2
# done
...
for S in $(seq 1 7); do 
   for reps in $(seq 1 3); do
      mpirun -x CUDA_VISIBLE_DEVICES=0,1 -np 4 -npernode 2 $VARS0 $VARS1 $VARS2 -host $HOST1,$HOST2 ./$BENCHMARK -X $SIZE -Y $SIZE -Z $SIZE -N $ITERS | tee -a $FILENAME
   done
   let SIZE=$SIZE*2
done
...
# for S in $(seq 1 7); do 
#    for reps in $(seq 1 3); do
#       mpirun -x CUDA_VISIBLE_DEVICES=0,1,2,3 -np 8 -npernode 4  $VARS0 $VARS1 $VARS2 -host $HOST1,$HOST2 ./$BENCHMARK -X $SIZE -Y $SIZE -Z $SIZE -N $ITERS | tee -a $FILENAME
#    done
#    let SIZE=$SIZE*2
# done
