...
static void* allocate_host_pinned(size_t size, size_t& real_size)//, bool host_write_optimized = false)
{
...
static ibv_mr* pin_rdma_memory(Transport* tport, size_t size, void* buf)
{
  ...
  debug("registering rdma memory on tport=%p, pd=%p, size=%d, buf=%p",
        tport, (tport ? tport->pd : nullptr), int(size), buf); 
...
#ifdef KOKKOS_ENABLE_CUDA
  unsigned int set = 1;
  cuda_safe(cuPointerSetAttribute(&set, CU_POINTER_ATTRIBUTE_SYNC_MEMOPS, (CUdeviceptr) buf));
#endif
  assert_ibv(buf);
  return pin_rdma_memory(tport, real_size, buf);
}
...
void
RdmaScatterGatherEngine::request_received(RdmaWorkRequest* req)
{
...
void
RdmaScatterGatherEngine::remote_window_start_reply(RemoteWindow* win)
{
  uint32_t window_offset = (uintptr_t(win) - uintptr_t(rx_remote_windows_mr->addr)) / sizeof(RemoteWindow);
  uint64_t idx = rx_block_request_ctr % queue_size;
  uint64_t trip_number = rx_block_request_ctr / queue_size;
  uint64_t request = MAKE_BLOCK_PACK_REQUEST(win->num_entries, win->requester, trip_number, window_offset);
  volatile_store(&rx_block_request_cmd_queue[idx], request);

  debug("starting reply %" PRIu64 " back to %d on token %" PRIu32 " on index %" PRIu64 " on window %u from %p to %p", 
        request, win->requester, win->reply_token, idx, window_offset, win->cfg.reply_tx_buf, win->cfg.reply_rx_buf);
  pending_replies.push(win);
  ++rx_block_request_ctr;
}

void
RdmaScatterGatherEngine::remote_window_finish_reply()
{
  ...
  sr->wr.rdma.remote_addr = (uint64_t) win->cfg.reply_rx_buf + win->offset * win->elem_size;
  sr->wr.rdma.rkey = win->cfg.reply_rx_key;

  debug("finishing reply back to %d on token %" PRIu32 " from %p to %p of size %d", 
        win->requester, win->reply_token, sge->addr, sr->wr.rdma.remote_addr, int(sge->length));
...
void
RdmaScatterGatherEngine::response_received(RdmaWorkRequest* req, uint32_t token)
{
...
void
RdmaScatterGatherEngine::send_remote_window(Transport* tport, int pe, uint32_t num_entries)
{
  ...
  sge[1].addr = (uint64_t) tx_element_request_queue_mr->addr + (pe*queue_size + offset)*sizeof(uint32_t);
  ...
  debug("request %p with %" PRIu32 " entries at offset=%" PRIu32 " on token=%" PRIu32 " from pe=%d",
...
void
RdmaScatterGatherEngine::poll(Transport* tport)
{
  ibv_wc wc;
  ...
  if (num_entries >= 1){
    ...
    switch(req->type){
      ...
      case RdmaWorkRequest::RECV_SG_RESPONSE: {
        ...
        //go ahead and put the request back into the queue
        time_safe(ibv_safe(ibv_post_srq_recv(req->srq, &req->req.rr, &req->bad_req.rr)));
        //ibv_safe(ibv_post_recv(req->qp, &req->req.rr, &req->bad_req.rr));
        break;
      }
...
void
RdmaScatterGatherEngine::poll_requests()
{
  while(is_running()){
...
void
RdmaScatterGatherEngine::poll_responses()
{
  while(is_running()){
...
void
RdmaScatterGatherEngine::generate_requests()
{
  while(is_running()){
...
void
RdmaScatterGatherEngine::check_for_new_block_requests()
{
  ...
  while (GET_BLOCK_FLAG(next_request) == ready_flag){
    ...
    debug("got block request of size %" PRIu32 " for pe %" PRIu32 " on idx %" PRIu64 " at offset %" PRIu64,
          num_entries, pe, tx_block_request_ctr, tx_element_request_sent_ctrs[pe]);
...
void
RdmaScatterGatherEngine::ack_response(PendingRdmaRequest& req)
{
  ...
  for (auto iter = misordered_acks.begin(); iter != misordered_acks.end(); ++iter){
    auto tmp = iter++;
...
RdmaScatterGatherEngine::RdmaScatterGatherEngine(MPI_Comm c, void* buf, size_t elem_size, size_t header_size) :
  comm(c),
  ...
  terminate_signal(0)
{
  ...
  tx_element_request_ctrs = (uint64_t*) allocate_device(num_pes*sizeof(uint64_t), ignore_actual_size);
  memset_device(tx_element_request_ctrs, 0, num_pes*sizeof(uint64_t));

  ack_ctrs_d = (uint64_t*) allocate_device(num_pes*sizeof(uint64_t), ignore_actual_size);
  memset_device(ack_ctrs_d, 0, num_pes*sizeof(uint64_t));

  tx_element_reply_ctrs = (uint64_t*) allocate_device(num_pes*sizeof(uint64_t), ignore_actual_size);
  memset_device(tx_element_reply_ctrs, 0, num_pes*sizeof(uint64_t));

  tx_element_request_trip_counts = (uint32_t*) allocate_device(queue_size*num_pes*sizeof(uint32_t), ignore_actual_size);
  memset_device(tx_element_request_trip_counts, 0, num_pes*queue_size*sizeof(uint32_t));

  tx_element_request_queue_mr = allocate_device_rdma_memory(response_tport, queue_size * num_pes * sizeof(uint32_t));
  memset_device(tx_element_request_queue_mr->addr, 0, num_pes*queue_size*sizeof(uint32_t));

  tx_element_aggregate_ctrs = (uint64_t*) allocate_device(num_pes*sizeof(uint64_t), ignore_actual_size);
  memset_device(tx_element_aggregate_ctrs, 0, num_pes*sizeof(uint64_t));

  ack_ctrs_h = (uint64_t*) allocate_host_pinned(num_pes*sizeof(uint64_t), ignore_actual_size);
  ...
  rx_element_reply_queue_mr = allocate_device_rdma_memory(response_tport, reply_size);
  tx_element_reply_queue_mr = allocate_device_rdma_memory(response_tport, reply_size);

  rx_element_request_queue_mr = allocate_device_rdma_memory(request_tport, NUM_RECV_WRS * queue_size * sizeof(uint32_t));
  rx_remote_windows_mr = allocate_host_rdma_memory(request_tport, NUM_RECV_WRS * sizeof(RemoteWindow));
  tx_remote_windows_mr = allocate_host_rdma_memory(request_tport, NUM_SEND_WRS * sizeof(RemoteWindow));
  available_tx_windows.fill_empty(NUM_SEND_WRS);
  for (int w=0; w < NUM_SEND_WRS; ++w){
    RemoteWindow* win = (RemoteWindow*) ((char*)tx_remote_windows_mr->addr + w*sizeof(RemoteWindow));
  ...
  if (request_tport->pd != response_tport->pd){
    Kokkos::abort("does not support request/response in different IBV protection domains");
  }

  // TODO
  // we are for now assuming that request/response tports are on the same pd
  all_request_mr = allocate_host_rdma_memory(request_tport, sizeof(RdmaWorkRequest) * TOTAL_NUM_WRS);
  ...
  available_recv_request_wrs.fill_from_storage(NUM_RECV_WRS, all_requests + START_RECV_REQUEST_WRS);
  available_send_response_wrs.fill_from_storage(NUM_SEND_WRS, all_requests + START_SEND_RESPONSE_WRS);
  available_recv_response_wrs.fill_from_storage(NUM_RECV_WRS, all_requests + START_RECV_RESPONSE_WRS);
  ...
  tx_block_request_cmd_queue = (uint64_t*) allocate_host_pinned(queue_size*sizeof(uint64_t), ignore_actual_size);
  rx_block_request_cmd_queue = (uint64_t*) allocate_host_pinned(queue_size*sizeof(uint64_t), ignore_actual_size);
  tx_block_reply_cmd_queue = (uint64_t*) allocate_host_pinned(queue_size*sizeof(uint64_t), ignore_actual_size);
  ...
  data.reply_tx_key = tx_element_reply_queue_mr->lkey; //I want my local key sent back to me
  ...
  MPI_Allgather(&data, sizeof(RdmaScatterGatherBuffer), MPI_BYTE,
                remote_bufs.data(), sizeof(RdmaScatterGatherBuffer), MPI_BYTE, comm);
  ...
  for (int pe=0; pe < num_pes; ++pe){
    if (::strcmp(data.hostname, remote_bufs[pe].hostname) == 0){
      ...
      if (pe != rank){
        char* peer_buf;
        cuda_safe(cuIpcOpenMemHandle((CUdeviceptr *)&peer_buf, remote_bufs[pe].handle,
  ...
  direct_ptrs_d = (void**) allocate_device(num_pes*sizeof(void*), ignore_actual_size);
  ...
  for (int pe=0; pe < num_pes; ++pe){
    ...
    cfg.reply_rx_buf = ((char*)rx_element_reply_queue_mr->addr + reply_buffer_stride*pe);
    cfg.reply_rx_key = rx_element_reply_queue_mr->rkey; // remote processes will put here, send remote key
    cfg.reply_tx_buf = ((char*)remote_bufs[pe].reply_tx_buffer + reply_buffer_stride*rank);
    cfg.reply_tx_key = remote_bufs[pe].reply_tx_key;
    debug("pe=%d tx_reply=%p:%" PRIu32 " rx_reply=%p:%" PRIu32,
          pe, cfg.reply_tx_buf, cfg.reply_tx_key, 
          cfg.reply_rx_buf, cfg.reply_rx_key);
  }
  ...
  for (int r=0; r < NUM_RECV_WRS; ++r){
    ...
    debug("post rr=%p of size=%d, key=%" PRIu32 ", addr=%p", 
          req, int(sge->length), sge->lkey, sge->addr);
  ...
  request_done_flag = (unsigned*) allocate_device(sizeof(unsigned)*2, ignore_actual_size);
  response_done_flag = request_done_flag + 1;
  fence_done_flag = (unsigned*) allocate_host_pinned(sizeof(unsigned), ignore_actual_size);
...
{
  RdmaScatterGatherEngine* sge = new RdmaScatterGatherEngine(comm, buf, elem_size, header_size);
  return sge;
}
