...
for S in $(seq 1 23); do 
   for reps in $(seq 1 3); do
      ...
      -x CUDA_VISIBLE_DEVICES=$DEVICE_ID_2 -np 1 $VARS0 $VARS1 $VARS2 -host $HOST2  ./$BENCHMARK -N $SIZE -I $ITERS -M 3 | tee -a $FILENAME | tee -a $FILENAME
   done
   let SIZE=$SIZE*2
done
...
for S in $(seq 1 23); do 
  for reps in $(seq 1 3); do
      ...
      -x CUDA_VISIBLE_DEVICES=$DEVICE_ID_2 -np 1 $VARS0 $VARS1 $VARS2 -host $HOST2  ./$BENCHMARK -N $SIZE -I $ITERS -M 2 | tee -a $FILENAME | tee -a $FILENAME
 done
 let SIZE=$SIZE*2
done
...
for S in $(seq 1 23); do 
 for reps in $(seq 1 3); do
      ...
      -x CUDA_VISIBLE_DEVICES=$DEVICE_ID_2 -np 1 $VARS0 $VARS1 $VARS2 -host $HOST2  ./$BENCHMARK -N $SIZE -I $ITERS -M 0 | tee -a $FILENAME | tee -a $FILENAME
 done
 let SIZE=$SIZE*2
done
...
for S in $(seq 1 23); do 
  for reps in $(seq 1 3); do
      ...
      -x CUDA_VISIBLE_DEVICES=$DEVICE_ID_2 -np 1 $VARS0 $VARS1 $VARS2 -host $HOST2  ./$BENCHMARK -N $SIZE -I $ITERS -M 1 | tee -a $FILENAME | tee -a $FILENAME
 done
 let SIZE=$SIZE*2
done
