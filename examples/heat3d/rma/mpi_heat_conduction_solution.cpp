...
struct System {
  ...
  System(MPI_Comm comm_) : comm(comm_) {
    ...
    X_hi               = X;
    Y_hi               = Y;
    Z_hi    = Z; /* is this a mistake in original? No, it is set in subdomain */
    N       = 10000;
    I       = 100;
  ...
  void setup_subdomain() {
    printf("Comm nx,ny,nz: (%i %i %i)\n", comm.nx, comm.ny, comm.nz);
    int dX =
    ...
    Z_lo   = dZ * comm.z;
    Z_hi   = Z_lo + dZ;
    if (Z_hi > Z) Z_hi = Z; /* It's Z_hi = X in the OG code, def a mistake */
    /* break the code with -X 20 -Y 20 -Z 201 and 4 procs. */
    ...
    if (Y_lo != Y) T_up = buffer_t("System::T_up", X_hi - X_lo, Z_hi - Z_lo);
    if (Z_lo != 0)
  ...
  void pack_T_halo() {
    ...
    if (Z_hi != Z) {
      ...
      mar++; /* why do we track mar here if we don't use it? */
    }
  }
